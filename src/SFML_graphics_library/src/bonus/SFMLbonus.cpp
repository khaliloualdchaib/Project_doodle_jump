...
SFMLDoodleJump::SFMLBonus::SFMLBonus(float width, float height, const std::tuple<float, float>& pos,
                                     const std::shared_ptr<sf::RenderWindow>& w)
    : SFMLEntity(width, height, pos, w)
{
}
...
void SFMLDoodleJump::SFMLBonus::setSprite(const sf::Sprite& s) { SFMLBonus::sprite = s; }
