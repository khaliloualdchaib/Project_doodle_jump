...
SFMLDoodleJump::Game::Game(const std::map<std::string, unsigned int>& windowconf)
{
        ...
        window = std::make_shared<sf::RenderWindow>(
            sf::VideoMode(windowConfiguration["Width"], windowConfiguration["Height"]), "DoodleJump");
}

void SFMLDoodleJump::Game::runGame()
{
        shared_ptr<DoodleJump::Camera> camera = make_shared<DoodleJump::Camera>(
            DoodleJump::Camera(windowConfiguration["Width"], windowConfiguration["Height"]));
        shared_ptr<SFMLDoodleJump::ConcreteFactory> factory =
            make_shared<SFMLDoodleJump::ConcreteFactory>(SFMLDoodleJump::ConcreteFactory(window));
        ...
        while (window->isOpen()) {
                ...
                if (DoodleJump::Stopwatch::getInstance().getTime_difference() >= 1 / 60.0f) {
                        ...
                        if (std::get<1>(world.getPlayer()->getPosition()) >= -3) {
                                ...
                                game_over.setPosition(sf::Vector2f(std::get<0>(camera->Transformation(-2, 0.5)),
                                                                   std::get<1>(camera->Transformation(-2, 0.5))));
