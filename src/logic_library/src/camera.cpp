...
float DoodleJump::Camera::TransformWidth(float w) const { return w * float(Camera::width / 8.0); }

float DoodleJump::Camera::TransformHeight(float h) const { return h * float(Camera::height / 6.0); }